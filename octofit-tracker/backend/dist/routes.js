...
router.get('/users', async (_request, response, next) => {
    try {
        response.json(await models_1.User.find().select('-passwordHash').sort({ name: 1 }));
    }
    catch (error) {
        next(error);
    }
...
router.post('/auth/register', async (request, response, next) => {
    try {
        ...
        const user = await models_1.User.create({
            ...
            grade,
            fitnessLevel,
            avatarColor: (0, auth_1.fallbackColor)(email),
        });
        response.status(201).json({ token: (0, auth_1.createToken)(user.id), user: { ...user.toObject(), passwordHash: undefined } });
    }
    catch (error) {
        next(error);
    }
...
router.post('/auth/login', async (request, response, next) => {
    try {
        const user = await models_1.User.findOne({ email: request.body.email });
        if (!user || !(0, auth_1.verifyPassword)(request.body.password || '', user.passwordHash)) {
            response.status(401).json({ message: 'Invalid email or password.' });
            return;
        }
        response.json({ token: (0, auth_1.createToken)(user.id), user: { ...user.toObject(), passwordHash: undefined } });
    }
    catch (error) {
        next(error);
    }
...
router.get('/activities', async (_request, response, next) => {
    try {
        response.json(await models_1.Activity.find().populate('user', 'name avatarColor').sort({ activityDate: -1 }));
    }
    catch (error) {
        next(error);
    }
...
router.post('/activities', async (request, response, next) => {
    try {
        ...
        const activity = await models_1.Activity.create({
            ...
            activityDate,
            notes,
            points: (0, auth_1.calculatePoints)(type, Number(duration), Number(distance)),
        });
...
router.delete('/activities/:id', async (request, response, next) => {
    try {
        const activity = await models_1.Activity.findByIdAndDelete(request.params.id);
...
router.get('/teams', async (_request, response, next) => {
    try {
        response.json(await models_1.Team.find().populate('members', 'name grade avatarColor').sort({ name: 1 }));
    }
    catch (error) {
        next(error);
    }
...
router.post('/teams', async (request, response, next) => {
    try {
        response.status(201).json(await models_1.Team.create(request.body));
    }
    catch (error) {
        next(error);
    }
...
router.get('/leaderboard', async (_request, response, next) => {
    try {
        const standings = await models_1.Activity.aggregate([
...
router.get('/workouts', async (request, response, next) => {
    try {
        ...
        response.json(await models_1.Workout.find(filter).sort({ duration: 1 }));
    }
    catch (error) {
        next(error);
    }
...
router.get('/workouts/recommended/:userId', async (request, response, next) => {
    try {
        const user = await models_1.User.findById(request.params.userId);
        if (!user) {
            response.status(404).json({ message: 'User not found.' });
            return;
        }
        response.json(await models_1.Workout.find({ level: user.fitnessLevel }).sort({ duration: 1 }).limit(3));
    }
    catch (error) {
        next(error);
    }
